...
GAUSSIAN_SMOOTH_FILTER_SIZE = (3, 3)  # Giảm kích thước để giữ chi tiết ký tự
ADAPTIVE_THRESH_BLOCK_SIZE = 15  # Giảm kích thước block
ADAPTIVE_THRESH_WEIGHT = 5  # Giảm weight để tăng độ nhạy
...
def maximizeContrast(imgGrayscale):
    ...
    # Giảm kích thước kernel để giữ chi tiết nhỏ
    structuringElement = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))

    # Giảm số lần lặp để tránh nhiễu
    imgTopHat = cv2.morphologyEx(imgGrayscale, cv2.MORPH_TOPHAT, structuringElement, iterations=6)
    imgBlackHat = cv2.morphologyEx(imgGrayscale, cv2.MORPH_BLACKHAT, structuringElement, iterations=6)
